...
class EBLAlignerTab:
    ...
    def start_alignment(self):
        ...
        # 1D 프로파일로 십자가의 "진짜 수학적 중심" 찾기 (비대칭 팔 길이로 인한 오프셋 완벽 차단)
        ...
        if max_x > 0 and max_y > 0:
            ...
            # 혹시 모를 Fallback (원본 Bounding Box 로직)
        ...
        # ★ 핵심 해결책: 템플릿 이미지 내부에서 진짜 십자가 교차점이 몇 픽셀 위치에 있는지 기억 ★
        ...
        threading.Thread(target=self._process_align_worker, args=(image_files, target_dir, template, template_cx, template_cy, suffix), daemon=True).start()

    def _process_align_worker(self, image_files, work_dir, template, template_cx, template_cy, suffix):
        ...
        for idx, img_path in enumerate(image_files):
            ...
            try:
                ...
                # 원본 오리지널 마커 인식 로직 (이물질 무시 & 정확한 형태 탐지)
                def find_mark_in_roi(roi_img, offset_x, offset_y):
                    ...
                    # ★ 템플릿의 가짜 중앙(tw/2.0)이 아닌 기록해둔 '진짜 중앙(template_cx)'으로 덧셈 보정
                    center_x = peak_x + offset_x + template_cx
                    center_y = peak_y + offset_y + template_cy
                ...
                # 완벽한 중앙 위치(LocalOrigin)와 회전각 추출을 위해 오리지널 getAffineTransform 사용
                ...
                # 이미지 정중앙에 해당하는 정확한 장비(UV) 좌표
                origin_uv = get_uv_from_pixel(w/2.0, h/2.0)
                
                # Raith 소프트웨어용 "순수 바운딩 박스" 크기 추출 (이중 회전 찌그러짐 원천 차단)
                dist_px_x = math.hypot(pixel_LR[0] - pixel_LL[0], pixel_LR[1] - pixel_LL[1])
                scale_x = 400.0 / dist_px_x
                
                dist_px_y = math.hypot(pixel_UL[0] - pixel_LL[0], pixel_UL[1] - pixel_LL[1])
                scale_y = 400.0 / dist_px_y
                
                width_uv = w * scale_x
                height_uv = h * scale_y
                
                # 중심점(Origin)을 기준으로 완벽한 직사각형 코너 생성
                # (이렇게 해야 Raith에서 RotationUV 적용 시 코너가 대각선 오차 없이 딱 맞아떨어짐)
                ll_mm = ((origin_uv[0] - width_uv / 2.0) / 1000.0, (origin_uv[1] - height_uv / 2.0) / 1000.0)
                ur_mm = ((origin_uv[0] + width_uv / 2.0) / 1000.0, (origin_uv[1] + height_uv / 2.0) / 1000.0)
                or_mm = (origin_uv[0] / 1000.0, origin_uv[1] / 1000.0)
...
"""
                ...
                self.root.after(0, self.log, f"[OK] {filename} -> Alignment complete")

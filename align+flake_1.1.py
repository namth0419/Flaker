...
def process_single_image(args):
    ...
    try:
        ...
        for cnt in contours:
            ...
            cv2.circle(img_result, center, max(drawn_radius_px, 3), (0, 0, 255), 3)
            cv2.drawMarker(img_result, center, (0, 255, 0), cv2.MARKER_CROSS, 10, 1)
            cv2.putText(img_result, f"#{flake_count} ({area_um2:.1f})", (center[0] + drawn_radius_px, center[1] - drawn_radius_px), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            
            if clr_method == "circle":
                cv2.circle(img_result, center, int(radius + clearance_px + halo_buffer_px), (0, 255, 255), 1)
            else:
...
class FlakeAnalyzerTab:
    ...
    def load_default_config(self):
        return {
            ...
            "display": {"circle_scale_percent": 100.0}
        }
...
class EBLAlignerTab:
    ...
    def build_ui(self):
        ...
        ttk.Checkbutton(dir_frame, text="Save debug overlay images (verify mark detection)",
                        variable=self.var_save_debug).grid(row=2, column=1, sticky=tk.W, padx=5, pady=2)
        ...
        inst_text = (
            ...
            "4. An OpenCV window will pop up showing the first image.\n"
            "5. Drag a bounding box around the SMALL crosshair mark.\n"
            "6. Press SPACE or ENTER to confirm the ROI.\n"
            "7. The system will automatically align all images and generate .ssc files."
        )
    ...
    def start_alignment(self):
        ...
        # [Fix 3] 빈 문자열 입력 시 크래시 방지 및 예외 처리
        if not suffix:
            messagebox.showerror("Error", "Target Suffix를 입력해주세요. (예: _ShiftN)")
            return
        ...
        if not contours:
            self.log("Error: Could not detect shape. Exiting.")
            self.btn_run.config(state=tk.NORMAL)
            return
        ...
        box_size = max(w_bound, h_bound) // 2 + 8 
        ...
        threading.Thread(target=self._process_align_worker, args=(image_files, target_dir, template, tw, th, suffix), daemon=True).start()

    def _process_align_worker(self, image_files, work_dir, template, tw, th, suffix):
        ...
        for idx, img_path in enumerate(image_files):
            ...
            try:
                ...
                def refine_centroid(roi_img, guess_x, guess_y, win=22):
                    # Refine a rough (template-match) location to the intensity
                    # centroid of the bright cross near that location.
                    # Returns (x, y) in ROI coordinates, or None if unreliable
                    # (caller then keeps the original template-match location).
                    rh, rw = roi_img.shape
                    x0, y0 = int(round(guess_x)), int(round(guess_y))
                    l, r = max(0, x0 - win), min(rw, x0 + win + 1)
                    t, b = max(0, y0 - win), min(rh, y0 + win + 1)
                    patch = roi_img[t:b, l:r].astype(np.float32)
                    if patch.size == 0:
                        return None
                    p8 = patch.astype(np.uint8)
                    thr, _ = cv2.threshold(p8, 0, 255,
                                           cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                    m8 = ((patch >= thr) * 255).astype(np.uint8)
                    # Pick the bright blob nearest the guess so nearby dust/marks
                    # cannot pull the centroid away from the real cross.
                    ...
                    cx = (xs * wmask).sum() / wmask.sum()
                    cy = (ys * wmask).sum() / wmask.sum()
                    return (l + cx, t + cy)
                ...
                def find_mark_in_roi(roi_img, offset_x, offset_y):
                    ...
                    # Template-match center estimate (original behavior).
                    match_x = peak_x + (tw / 2.0)
                    match_y = peak_y + (th / 2.0)

                    # Centroid refinement: locate the true cross center independent of
                    # where the user first clicked, improving run-to-run reproducibility.
                    refined = refine_centroid(roi_img, match_x, match_y)
                    if refined is not None:
                        rcx, rcy = refined
                        # Sanity clamp: if the centroid disagrees with the template
                        # match by more than the search window, distrust it and keep
                        # the template-match value (protects against bad refinement).
                        if math.hypot(rcx - match_x, rcy - match_y) <= 22:
                            final_x, final_y = rcx, rcy
                        else:
                            final_x, final_y = match_x, match_y
                    else:
                        final_x, final_y = match_x, match_y
                ...
                pixel_LL = find_mark_in_roi(roi_LL, offset_x=0, offset_y=h - y_cut)

                roi_LR = img_gray[h - y_cut : h, w - x_cut : w]
                pixel_LR = find_mark_in_roi(roi_LR, offset_x=w - x_cut, offset_y=h - y_cut)

                roi_UL = img_gray[0 : y_cut, 0 : x_cut]
                pixel_UL = find_mark_in_roi(roi_UL, offset_x=0, offset_y=0)

                nominal_uv = get_mark_nominal_uv(filename, suffix)
                
                src_pts = np.float32([pixel_LL, pixel_LR, pixel_UL])
                dst_pts = np.float32([nominal_uv['LL'], nominal_uv['LR'], nominal_uv['UL']])
                
                affine_matrix = cv2.getAffineTransform(src_pts, dst_pts)

                # Diagnostic: RMS residual of the three marks (stage units).
                # Does not affect output; lets alignment quality be compared before
                # and after the centroid refinement.
                src_h = np.hstack([src_pts, np.ones((3, 1), dtype=np.float32)])
                mapped = (affine_matrix @ src_h.T).T
                residuals = np.linalg.norm(mapped - dst_pts, axis=1)
                rms_residual = float(np.sqrt(np.mean(residuals ** 2)))

                angle_rad = math.atan2(affine_matrix[1, 0], affine_matrix[0, 0])
                angle_deg = math.degrees(angle_rad)
                
                def get_uv_from_pixel(px, py):
                    pt = np.array([px, py, 1])
                    res = np.dot(affine_matrix, pt)
                    return res[0], res[1]
                
                lower_left_uv = get_uv_from_pixel(0, h)
                upper_right_uv = get_uv_from_pixel(w, 0)
                origin_uv = get_uv_from_pixel(w/2, h/2)
                
                ll_mm = (lower_left_uv[0] / 1000, lower_left_uv[1] / 1000)
                ur_mm = (upper_right_uv[0] / 1000, upper_right_uv[1] / 1000)
                or_mm = (origin_uv[0] / 1000, origin_uv[1] / 1000)
                
                # [Fix 1] 원본 파일 파괴를 막기 위한 os.path.splitext 활용
                base_name, _ = os.path.splitext(filename)
                bmp_filename = f"{base_name}.bmp"
                ssc_filename = f"{base_name}.ssc"
                
                bmp_path = os.path.join(work_dir, bmp_filename)
                cv2.imwrite(bmp_path, img)
                
                ssc_content = f"""[SLOWSCAN]
Bitmap={bmp_filename}
LowerLeftUV={ll_mm[0]:.7f},{ll_mm[1]:.7f}
UpperRightUV={ur_mm[0]:.7f},{ur_mm[1]:.7f}
LocalOriginUV={or_mm[0]:.7f},{or_mm[1]:.7f}
StageUV=0.000000,0.000000
RotationUV={angle_deg:.5f}
"""
                with open(os.path.join(work_dir, ssc_filename), 'w') as f:
                    f.write(ssc_content)

                # Debug overlay: draw the three detected mark centers so the
                # detection can be visually verified against the real crosses.
                # Written to a 'debug_align' subfolder; never overwrites inputs and
                # has no effect on the .ssc output.
                if self.var_save_debug.get():
                    dbg = img.copy()
                    for (px, py), name in zip(
                            [pixel_LL, pixel_LR, pixel_UL], ['LL', 'LR', 'UL']):
                        ipx, ipy = int(round(px)), int(round(py))
                        cv2.drawMarker(dbg, (ipx, ipy), (0, 0, 255),
                                       cv2.MARKER_CROSS, 40, 2)
                        cv2.circle(dbg, (ipx, ipy), 14, (0, 255, 0), 2)
                        cv2.putText(dbg, name, (ipx + 16, ipy - 16),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    cv2.putText(dbg, f"residual RMS={rms_residual:.2f}  rot={angle_deg:.2f}deg",
                ...
                self.root.after(0, self.log,
                                f"[OK] {filename} -> residual RMS={rms_residual:.3f}, "
                                f"rot={angle_deg:.3f} deg")
